...
// From 0.4.16
// - Non IE stuff, this file is only for IE
// + Hacks for Harmony CSS
// + User smileys
// From 0.4.17
// ! Hack for Harmony CSS
...
with_jquery(function ($) {
	...
	var debug = true, scriptVersion = '0.4.17.1' + (debug ? ' (debug)' : '');
	...
	var prepareShoutbox = function () {
		...
		appendUserCSS(".mod_shoutbox .shout_rowalt.highlight { background-color: #BFD !important; } .mod_shoutbox .shout_rowalt.highlight_quote { background-color: #FFA !important; } #SHOUT_MESSAGE { overflow-x: hidden; } #notification { position: absolute; height: 0px; width: 0px; } ");
		$('#TQC_SHOUT_MESSAGE').clone().appendTo("#mod_shoutbox").hide();
		$('#TQC_SHOUT_MESSAGE').attr("id", "SHOUT_MESSAGE").empty();
	...
	var shoutBox_OnChange = function (event, request, options) {
		...
		$($("#TQC_SHOUT_MESSAGE ul").get().reverse()).each(function () {
			...
			if(optionsDB.get("hidesmileys") || optionsDB.get("hideimages")) {
					...
					if(aLink.attr("class") && aLink.attr("class").indexOf("class_") != -1) {
						...
						if(optionsDB.get("highlightuser")) {
							aLink.hover(function () {
								$(".u_" + secureNick).addClass("highlight");
							}, function () {
								$(".u_" + secureNick).removeClass("highlight");
							});
						}
		...
		if(optionsDB.get("notification") && notifySound) {
	...
	var addTextToShoutbox = function (from, fromLink, fromColorClass, htmlContent) {
		...
		var scrollUp = 0;
		if($("#SHOUT_MESSAGE ul").length > optionsDB.get("messages_to_display")) {
			var firstMessage = $("#SHOUT_MESSAGE ul").last();
			if(optionsDB.get("revertshout")) {
				firstMessage = $("#SHOUT_MESSAGE ul").first();
			}
	...
	var prepareUserList = function () {
		...
		appendUserCSS(".frame_list { overflow-x: hidden !important; } #user_mod { width: " + $(".frame_list").width() + "px; height: " + $(".frame_list").height() + "px; float: right; } #searchbox { width: 126px; height: 14px } #search_result { height: " + $(".frame_list").height() + "px; width: " + $(".frame_list").width() + "px; overflow: auto; } #search_result a { line-height: 20px; padding: 0 10px 0 10px; display: block; } #nConnected { padding: 0 10px 0 10px; } .user_change_enter { position: relative; left: " + $(".frame_list").width() + "px; } .user_change_gone { position: relative; left: 10px; } #context_menu { position: absolute; z-index: 9000; border: 1px solid #222; background-color: #CCC; } #context_head { font-weight: bold; background-color: #BBB; border-bottom: 1px solid #AAA; padding: 2px; text-align: center; cursor: default; } .context_option { border-bottom: 1px solid #AAA; padding: 2px; } .context_option a { cursor: pointer; } .user_ignored { text-decoration: line-through; } .user_friend { font-style: italic; } .user_connected { color: lime; } .user_disconnected { color: red; } .selected_option { font-style: italic; }");
		if(optionsDB.get("inshoutmp")) {
			appendUserCSS(".mp_frame { padding: 10px; width: auto; height: auto; position: absolute; display: block; z-index: 9000; top: " + MPTopPos + "px; left: " + MPLeftPos + "px; background-color: #EEE; color: #111; border-radius: 15px; border: 2px solid #222; max-width: 640px; } .mp_subject { margin-top: 4px; width: 100%; padding-right: 4px;} .mp_text { margin-top: 4px; } #mp_subject_input { width: 100%; box-sizing: border-box; } #mp_text_input { width: 335px; height: 120px; box-sizing: border-box; } .mp_buttons { text-align: center; margin-top: 12px; } #reply_mp_frame { padding-top: 8px; margin-top: 8px; border-top: 1px solid #666} .mp_frame .confirm { text-align: center; font-style: italic; color: green; display: none; } ");
		}

		// Embed the user list & make some space for N connected users
		$("#mod_shoutbox").prepend('<div id="user_mod"></div>');
		$("#user_mod").append('<div id="nConnected">0 users connectés</div>');
		$("#user_mod").append(userList).mouseleave(function () {
			if($("#searchbox")) {
				$("#nConnected").text(nUsersBeforeUpdate + " users connectés");
			}
		...
		$("#nConnected").dblclick(function () {
			...
			$("#searchbox").bind("keyup", function () {
				...
				if(!$("#search_result").length) {
					$(".frame_list").clone().appendTo("#user_mod").removeClass("frame_list").attr("id", "search_result");
				}
	...
	var userList_OnChange = function (event, request, options) {
		...
		if(!$("#nConnected input").length) {
			$("#nConnected").text(nUsers + " users connectés");
		}

		var newUserList = {};
		$(".frame_list a").each(function () {
			var secureNick =  $(this).text().toLowerCase().replace(".", "_").replace("● ", "");
			$(this).attr("id", "u_" + secureNick);
			newUserList[secureNick] = $(this).clone();
			if(oldUserList[secureNick] == null) {
				$("#user_change").append($(this).clone().text("+ " + $(this).text()).addClass("user_change_enter").delay(1).animate({"left": 10}, 1800));
			}
		...
		if(optionsDB.get("banlist")) {
			$.each(userDB.users, function (secureNick, data) {
				if(data.friend) {
					var aFriend = $('<a href="' + data.url + '" id="f_' + secureNick + '" class="' + data.classId + ' user_friend' + (data.ignore ? ' user_ignored' : '') + '"><span class="user_' + (newUserList[secureNick] == null ? 'disconnected' : 'connected' ) + '">●</span> ' + data.userName + '</a>');
		...
		$.each(oldUserList, function (secureNick, userData) {
			if(newUserList[secureNick] == null) {
				$("#user_change").append(userData.clone().text("- " + userData.text()).addClass("user_change_gone").delay(800).animate({"left": hiddenPos}, 1800));
			}
		});
	...
	var createContextMenu = function (userE) {
		userE.click(function (e) {
			...
			var secureNick = userE.text().toLowerCase().replace(".", "_").replace("● ", "");
			...
			if(optionsDB.get("inshoutmp")) {
				$("#write_mp").click(function (e2) {
					...
					$("#context_menu").remove();
					if(createMPSendFrame(userE.attr("href").substring(27), userName)) {
						return false;
					}
					return true;
				});
			...
			$("#context_friend").click(function () {
				if(userDB.isFriend(secureNick)) {
					...
					var aFriend = $('<a href="' + userDB.users[secureNick].url + '" id="f_' + secureNick + '" class="' + userDB.users[secureNick].classId + ' user_friend' + (userDB.users[secureNick].ignore ? ' user_ignored' : '') + '"><span class="user_connected">●</span> ' + userDB.users[secureNick].userName + '</a>');
					$("#user_friend_list").append(aFriend);
					createContextMenu(aFriend);
	...
	var shoutBoxText_OnKeyDown = function (e) {
		...
		if(e.which == autocompleteKey) {
			if($(this)[0].selectionStart != $(this)[0].selectionEnd && $(this)[0].selectionEnd == $(this).val().length && selectedUserName.length) {
				$(this).val($(this).val().substring(0, ($(this)[0].selectionEnd - selectedUserName.length)).concat(selectedUserName) + (optionsDB.get("addspaceafterautoc") ? ' ' : ''));
			}
			$(this)[0].selectionStart = $(this).val().length;
			e.preventDefault();
		}
		if(e.which == 13 && $(this)[0].selectionStart != $(this)[0].selectionEnd && $(this)[0].selectionEnd == $(this).val().length && selectedUserName.length) {
	...
	//////////////////////////////////////////////////
	var getUserByNamePartial = function (name) {
		var usersTab = Array();
		$.each(oldUserList, function (secureNick, userData) {
			if(userData.text().toLowerCase().indexOf(name) == 0) {
				usersTab.push({'secureNick':secureNick, 'name': userData.text()});
			}
		});
		return usersTab;
	};
	...
	var setResizer = function () { 
		appendUserCSS('#mod_shoutbox { height:  ' + optionsDB.get("shoutbox_height") + 'px; } #MAIN { width: 90%; } #top_content .arian_nav { width: 90%; } .mod_shoutbox .frame { width: auto; height : 100%; } .mod_shoutbox .frame_list { width: 200px; height : 100%; } .mod_shoutbox .form { ' + (isHarmonyCss ? 'width: 98.7%' : 'height: 32px; padding: 4px 6px 4px 4px;') + ' } #shout_text { width: 100%; } .big_box h1 { width: ' + (isHarmonyCss ? '100.7%' : '100%') + '; } ');

		if(optionsDB.get("bbcode")) {
			minSBHeight += bbcodeHeight;
		}
	...
	var resizeShoutbox = function () {
		var height = $("#mod_shoutbox").height() - 42 - bbcodeHeight;
		if(optionsDB.get("shoutbox")) {
			$("#SHOUT_MESSAGE").height(height);
		}
		else {
			$("#TQC_SHOUT_MESSAGE").height(height);
		}

		if(optionsDB.get("userlist")) {
			$("#user_mod").height(height);
		}
	...
	var addBBcodeBar = function () {
		bbcodeHeight = 26;
		appendUserCSS('#bbcode_bar { height: ' + bbcodeHeight + 'px; border-top: 1px solid #DDD; line-height: 18px; }' +
			'.bbcode_bar { padding: 4px; float: left; }' +
			'.bbcode_separator {float: left; background-color: #DDD; padding-top: ' + bbcodeHeight + 'px; padding-left: 1px; margin-right: 4px; margin-left: 4px}' +
			...
			'.bbcode_usersmiley { cursor: pointer; margin-right: 2px; margin-left: 2px; } #usersmiley_management { margin-left: 2px; } ' + 
			'.mod_shoutbox .form { padding: 0; } ');

		
		$("#mod_shoutbox").append('<div id="bbcode_bar"></div>');
		var divs = ["bbcode", "color", "smiley", "command"];
		if(optionsDB.get("usersmiley")) { divs.push("usersmiley"); }
		$.each(divs, function (k,v) {
			$("#bbcode_bar").append('<div id="bbcode_' + v + '" class="bbcode_bar"></div>');
			if(k != divs.length - 1) {
				$("#bbcode_bar").append('<div class="bbcode_separator"></div>');
			}
		});
		...
		if(optionsDB.get("usersmiley")) {
			...
			$("#bbcode_usersmiley").append('<select id="usersmiley_management"><option value="default">--Smileys--</option><option class="add_smiley">Ajouter</option><option class="del_all_smiley">Tout supprimer</option></select>');
			$.each(userData.getAll("smiley"), function (k,v) {
				$("#usersmiley_management").append('<option class="del_smiley">Supprimer ' + k + '</option> ');
			});

			$("#usersmiley_management").change(function() {
				var selectedOption = $("#usersmiley_management option:selected");
				$("#usersmiley_management option[value='default']").attr("selected", "selected");
				if(selectedOption.hasClass("add_smiley")) {
					if($("#usersmiley_management option").length >= (optionsDB.get("autoresize") ? 18 : 4)) {
						alert("Il y a trop de smileys !");
						return;
					}
					var url = prompt("Url du smiley :");
					if(url == ""|| url === null ||  url.indexOf("http://") == -1) { return; }

					var nom = prompt("Nom du smiley :");
					if(nom == ""  || nom === null) { return; }
					...
					if(userData.get("smiley", nom) == undefined) {
						...
						img.onload = function() {
							...
							$("#usersmiley_management").append('<option class="del_smiley">Supprimer ' + nom + '</option> ');
							var new_smiley = $(' <img src="' + url + '" width="16" height="16" alt="' + nom + '" class="bbcode_usersmiley" /> ');
							$("#bbcode_usersmiley").prepend(new_smiley);
							new_smiley.click(function() {
				...
				else if(selectedOption.hasClass("del_smiley")) {
					var nomString = selectedOption.val().substring(10);
					userData.unset("smiley", nomString);
					$(".bbcode_usersmiley").each(function() {
						if($(this).attr("alt") == nomString) {
							$(this).remove();
							return false;
						}
					});
				...
				else if(selectedOption.hasClass("del_all_smiley")) {
					userData.clearData("smiley");
					$(".bbcode_usersmiley").each(function() {
						$(this).remove();
					});
					$("#usersmiley_management option").each(function() {
						if($(this).hasClass("del_smiley")) {
							$(this).remove();
						}
					});
				}
	...
	var hideGrades = function () { 
		...
		if(isHarmonyCss) {
			appendUserCSS('#MAIN { padding: 5px 0 0 0; } ');
		}
	};
	...
	//////////////////////////////////////////////////////////
	var setUnreadMessageTitle = function () { 
		appendUserCSS(".mod_shoutbox .shout_rowalt.highlight_nonread { background-color: #FAC !important; } ");
		var focus = true;
		$(window).focus(function () {
			if(!focus && messageCount > 0) {
		...
		$.ajax({
			...
			success: function(data) {
				...
				if($("#send_mp_frame").length) {
					$("#receive_mp_frame").offset({top: $("#send_mp_frame").offset().top + $("#send_mp_frame").height() + 36, left: MPLeftPos});
				}
			}
		});
		return true;
	...
	var createMPSendFrame = function (userId, userName) {
		...
		$("#website").append('<div id="send_mp_frame" class="mp_frame"><div class="mp_to">A : <b>' + userName + '</b></div><div class="mp_subject"><input type="text" id="mp_subject_input"/></div><div class="mp_text"><textarea id="mp_text_input"></textarea></div><div class="mp_buttons" id="send_mp_buttons"><input type="button" id="send_mp" value=" Envoyer le MP " /> <input type="button" id="cancel_mp" value=" Annuler " /></div><div class="confirm">Message envoyé !</div></div>');
		...
		if($("#receive_mp_frame").length) {
			$("#send_mp_frame").offset({top: $("#receive_mp_frame").offset().top + $("#receive_mp_frame").height() + 36, left: MPLeftPos});
			$("#mp_text_input").width($("#receive_mp_frame").width());
		}
		return true;
	};
	...
	///////////////////////
	var playNotification = function (type) {
		var soundFile;
		if(type == "quote") {
		...
		$("#website").append('<embed id="notification" type="application/x-shockwave-flash" flashvars="audioUrl=https://thetabx.net/download/' + soundFile + '&autoPlay=true" src="https://www.google.com/reader/ui/3523697345-audio-player.swf" width="0" height="0" quality="best"></embed>');
	...
	var sendStatistics = function () {
		...
		xdr.onload = function () {
			...
			if(lastVersion != "OK" && lastVersion != "KO") {
	...
	var optionsPanelCreator = function () {
		...
		appendUserCSS("#options_panel fieldset { border: 2px groove threedface; padding: 6px; } #options_panel legend { color: #111; } #options_panel a:hover { color: #FFF; } #options_panel { padding: 10px; width: auto; height: auto; position: absolute; display: block; z-index: 9000; top: 200px; left: 200px; background-color: #EEE; color: #111; border-radius: 15px; border: 2px solid #222; } ");
		$("#open_options_panel").click(function () {
			if($("#options_panel").length) {
				$("#options_panel").fadeOut(600, function () { $(this).remove(); }); 
				return false;
			}
			$("#website").append('<div id="options_panel"><h3><center>Options FTDB Shoutbox Mod</center></h3>' +
			...
			$.each(optionsDB, function (option, data) { $("#check_" + option).trigger("change"); });
			...
			$("#save_options_panel").click(function () {
				$.each(optionsDB, function (option, data) {
					...
					var value = true;
					if(data.type == "check") {
						$.each(optionsDB[option].requires, function (i, check) {
							if(! $(check).is(":checked")) {
								value = false;
							}
						});
						...
						value = $(optionsDB[option].requires[0]).val();
						if(!isNaN(value) && Number(value) >= optionsDB[option].minVal && Number(value) <= optionsDB[option].maxVal) {
	...
	var appendUserCSS = function (css) {
		if(!$("#mod_shoutbox_js").length) {
			$("head").append('<style id="mod_shoutbox_js"></style>');
		}
		$("#mod_shoutbox_js").append(css);
	};
	...
	var optionsDB = {
		...
		notification: {defaultVal: false, type: "check", requires: ["#check_notification", "#check_highlightquote", "#check_shoutbox"], frame: "#option_shoutbox", text: 'Son de notification lorsque vous êtes cité', reqLast: true},
		...
		soundmp: {defaultVal: false, type: "check", requires: ["#check_soundmp", "#check_blinkmp"], frame: "#option_other", text: 'Notification sonore à la réception de MPs', reqLast: true},
		inshoutmp: {defaultVal: false, type: "check", requires: ["#check_inshoutmp", "#check_blinkmp"], frame: "#option_other", text: 'Lire/Répondre/Créer des MP depuis la shoutbox', reqLast: true},
		...
		get: function (k) {
			if(this[k].val == undefined) {
				this[k].val = GM_getValue(k);
				if(this[k].val == undefined) {
					this[k].val = this[k].defaultVal;
				}
				this[k].val = (this[k].type == "number" ? Number(this[k].val) : this[k].val);
			}
			return this[k].val;
		}
	};
	...
	var isHarmonyCss = ($('select[name="css_select"]').val() == 3192);
	userDB.loadUsers();
	if(optionsDB.get("bbcode")) {
		addBBcodeBar();
	}
	...
	if(optionsDB.get("font") != "Par défaut") {
		appendUserCSS("#mod_shoutbox { font-family: " + optionsDB.get("font") + "; } ");
	}
	optionsPanelCreator();
});