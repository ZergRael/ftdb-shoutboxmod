...
with_jquery(function ($) {
	...
	// General info stuff
	var debug = false, revision = 75, scriptVersion = '0.7.6';
	...
	var createUserLink = function (user) {
		...
		if(optionsDB.get("userstats")) {
			userA.mouseenter(function() {
				...
				var xdr8 = new XDomainRequest();
				xdr8.open('GET', urls.userstats + user.hash + '/');
				xdr8.onload = function () {
					var data = JSON.parse(xdr8.responseText);
					if (data.hash) {
						title = "";
